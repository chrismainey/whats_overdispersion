...
_well, we need to be able to test for it first..._
```{r disptest}
# overdispersion test
...
'Sandwich' estimators can be use to scale the standard error, e.g. `sandwich` package
...
---
# 3b scaleing variance: Quasi-likehood
+ More relaxed assumption allowing scaling.
...
+ ___Negative Binomial___ `r Citep(bib, "hilbe2011negative", .opts = list(cite.style = "authoryear", style="html", max.names=2, longnamesfirst = FALSE))` Can be used as Poisson with addition gamma or normal distributed assumption (more on this with random effect...)
...
+ Spiegelhalter showed how to apply them in hospital indicator context, including funnel plots  `r Citep(bib, c("spiegelhalter_statistical_2012", "spiegelhalterHandlingOverdispersionPerformance2005"), .opts = list(cite.style = "authoryear", style="html", max.names=2, longnamesfirst = FALSE))`
...
### Taking the readmissions from the start:
...
+ If you can, get more data and inspect for aggregation, outlier etc.

+ Add more parameters if possible

+ Scale the variance simply with quasi models, or reporting dispersion ratio.

+ Other adjustment are all takes on the clustering / within and between variance concept.
